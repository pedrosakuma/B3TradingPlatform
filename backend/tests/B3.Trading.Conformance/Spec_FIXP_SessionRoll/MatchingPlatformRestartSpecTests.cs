...
public class MatchingPlatformRestartSpecTests
{
    ...
    public async Task Restart_Renegotiates_SurvivingOrderFlaggedStale_BookSurvives_FreshTradingRecovers()
    {
        ...
        var probeClOrdIds = new HashSet<ulong>();
        ...
        var restingClOrdId = await SessionRollSpecSupport.SubmitOrderAsync(http, userAuth, "ITUB4", survivorPrice);
        ...
        await docker.RestartMatchingAsync(
            ...
            whileRestarting: async () =>
            {
                ...
                {
                    var probeClOrdId = await SessionRollSpecSupport.StimulateGatewayWriteAsync(
                        http, userAuth, "VALE3", 60.00m, side: "Sell");
                    if (probeClOrdId is { } acceptedProbeClOrdId)
                        probeClOrdIds.Add(acceptedProbeClOrdId);
        ...
        foreach (var probeClOrdId in probeClOrdIds)
            await SessionRollSpecSupport.CancelOrderIfPresentAsync(http, userAuth, probeClOrdId);

        // Contract-level proof that matching's own book/WAL survived the
        // process restart: the pre-restart resting order is no phantom. Once
        // the FIXP session renegotiates, crossing it from the host still
        // yields a real terminal ER for the original order.
        var proofCrossStartedUtc = DateTimeOffset.UtcNow;
        var crossingClOrdId = await SessionRollSpecSupport.SubmitOrderAsync(
            http, userAuth, "ITUB4", survivorPrice, side: "Sell");
        var filledResting = await SessionRollSpecSupport.WaitForOrderAsync(http, userAuth, restingClOrdId, order =>
                order.Status == "Filled"
        ...
        var filledCrossing = await SessionRollSpecSupport.WaitForOrderAsync(http, userAuth, crossingClOrdId, order =>
                order.Status == "Filled"
                && order.CumulativeQuantity == SessionRollSpecSupport.RoundTripQuantity,
            SessionRollSpecSupport.TradeTimeout,
        ...
        await docker.WaitForMarketDataTradeDrainAsync(proofCrossStartedUtc, SessionRollSpecSupport.TradeTimeout);

        await SessionRollSpecSupport.AssertPostRecoveryTradingRoundTripAsync(
            http, userAuth, docker, "PETR4", freshRoundTripPrice, SessionRollSpecSupport.RoundTripQuantity);
    }
}
