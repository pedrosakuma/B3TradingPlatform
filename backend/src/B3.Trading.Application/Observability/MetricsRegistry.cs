...
public static class MetricsRegistry
{
    ...
    /// string identical to the live <c>AuditEventsTotal</c> tag);
    /// <c>reason</c> (<c>wal_backpressure</c> | <c>exception</c>).
    /// Distinct from <c>trading.wal.backpressure{call_site}</c>:
    /// drops here are audit envelopes that did not reach the WAL,
    /// regardless of the WAL's own backpressure metering. Dashboards
    /// and alerts that need to gate "did we lose audit coverage" on a
    /// path should target THIS counter and treat any sustained
    /// non-zero rate as an alert condition (the call site keeps the
    /// caller's response unaffected by audit failure by design — see
    /// <see cref="Audit.IAuditLogger"/> remarks for the two-mode
    /// contract).
    /// </summary>
    public static readonly Counter<long> AuditDropped =
        Meter.CreateCounter<long>("trading.audit.dropped_total");

    /// <summary>
    /// #607. Counts Entra external-token exchange attempts. Tags are bounded:
    /// <c>result</c> (success/failure), stable <c>reason</c>, and configured
    /// <c>issuer_alias</c>. No token, raw external subject or user label.
    /// </summary>
    public static readonly Counter<long> AuthExchangeRequests =
    ...
    public static readonly Histogram<double> AuthExchangeDurationSeconds =
        Meter.CreateHistogram<double>("trading.auth.exchange.duration_seconds");
